...
def run_program(bin, input_string="", memlim_gb=1024, silent=False):
	...
	def set_memory_limit():
		...
		resource.setrlimit(resource.RLIMIT_AS, (int(memlim_gb*1024*1024*1024), hard))
...
arg_parser.add_argument("--memlimit", "-M", help="Sets a memory limit in GB (default 2GB).", default=2.0)
arg_parser.add_argument("--silent", "-S", help="Do not print the stderr stream of the experiments to the screen.", action="store_true")
arg_parser.add_argument("--logstderr", "-L", help="Log the stderr stream of the experiments to the output file.", action="store_true")
...
def run_experiment(experiment, instance, solutions):
	...
	if use_valgrind: executable = ["valgrind", "--track-origins=yes", "--leak-check=full", executable_path]
	...
	return {
		...
		"stderr": result["stderr"] if log_stderr else "deactivated", 
		"stdout": stdout_json, 
...
#		output_file_name = F"{datetime.date.today()}-{experiment_file_name}.json"
		output_file_name = F"{datetime.datetime.now().isoformat(timespec='seconds')}-{experiment_file_name}.json"
