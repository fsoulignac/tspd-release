...
template<unsigned long N, typename Iterable>
std::bitset<N> create_bitset(const Iterable& numbers)
{
	std::bitset<N> b;
	for (auto n: numbers) b.set(n);
	return b;
}
...
inline std::bitset<N> intersection(const std::bitset<N>& b1, const std::initializer_list<T>& numbers)
{
...
template<unsigned long N, typename T>
inline std::bitset<N> unite(const std::bitset<N>& b1, const std::initializer_list<T>& numbers)
{
	return b1 | create_bitset<N>(numbers);
}
...
{
	j = std::vector<size_t>();
	for (auto i = 0ul; i < b.size(); ++i) if (b.test(i)) j.push_back(i);
}

template<unsigned long N>
void from_json(const nlohmann::json& j, std::bitset<N>& b)
{
	for (auto i: j) b.set(i);
}
} // namespace std
